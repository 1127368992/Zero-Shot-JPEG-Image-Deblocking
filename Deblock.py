...
parser = argparse.ArgumentParser('Zero-Shot Image Deblocking (Detail-Enhanced)')
...
parser.add_argument('--ws', default=24, type=int, help='Smaller window for precise similar blocks')
parser.add_argument('--bs', default=8, type=int, help='Block size (8 for JPEG)')
parser.add_argument('--nn', default=8, type=int, help='Fewer neighbors (reduce blur)')
parser.add_argument('--mm', default=4, type=int, help='Fewer blocks (reduce over-smoothing)')
parser.add_argument('--qf', default=20, type=int, help='Quality factor for JPEG compression')
parser.add_argument('--loss', default='L1', type=str, help='Loss function type')
parser.add_argument('--residual_weight', default=0.8, type=float, help='Residual weight (enhance details)')
...
def construct_block_bank():
    ...
    for image_file in image_files:
        ...
        img_clean = transform(img).unsqueeze(0)
        ...
        num_blk_w = img_blocked.shape[-1] // blk_sz  # 256//32=8
        num_blk_h = img_blocked.shape[-2] // blk_sz  # 256//32=8
        ...
        for blk_i in range(num_blk_w):
            for blk_j in range(num_blk_h):
                ...
                topk = torch.gather(patch_center, dim=-3,
                                    index=sort_indices.unsqueeze(1).repeat(1, 1, 1, 1, 1))
        ...
        print(f"Processed {image_file} in {elapsed:.2f}s. Block bank shape: {topk.shape}")
        file_name_without_ext = os.path.splitext(image_file)[0]
        np.save(os.path.join(bank_dir, file_name_without_ext), topk.cpu())

    print("Block bank construction completed.")


class DeblockNetwork(nn.Module):
    def __init__(self, n_chan=1, chan_embed=32):  
        super().__init__()
        self.act = nn.LeakyReLU(negative_slope=0.1, inplace=True)  
        self.conv1 = nn.Conv2d(n_chan, chan_embed, 3, padding=1)
        self.conv2 = nn.Conv2d(chan_embed, chan_embed, 3, padding=1)
        self.conv3 = nn.Conv2d(chan_embed, n_chan, 3, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        return torch.clamp(x * (1 - residual_weight) + residual * residual_weight, 0, 1)
...
def train(model, optimizer, block_bank):
    ...
    block1 = torch.gather(block_bank, 0, index1_exp).permute(0, 3, 1, 2)
    ...
    block2 = torch.gather(block_bank, 0, index2_exp).permute(0, 3, 1, 2)
...
def deblock_images():
    ...
    for image_file in image_files:
        ...
        model = DeblockNetwork(n_chan=1).to(device)
        print(f"Params for {image_file}: {sum(p.numel() for p in model.parameters() if p.requires_grad)}")

        optimizer = optim.AdamW(model.parameters(), lr=lr)
        scheduler = MultiStepLR(optimizer, milestones=[500], gamma=0.5)
        ...
        out_img_pil = to_pil_image(out_img.squeeze(0).cpu())  # 灰度图直接转换
